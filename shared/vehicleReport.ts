...
/** Where the discovered-sources list is cached; each app plugs in its own storage. */
export const reportCache: { get: (key: string) => string | null; set: (key: string, value: string) => void } = {
  get: () => null,
  set: () => {},
}
...
function readCache(): SourceDef[] | null {
  try {
    const raw = reportCache.get(CACHE_KEY)
    if (!raw) return null
...
function writeCache(sources: SourceDef[]) {
  try {
    reportCache.set(CACHE_KEY, JSON.stringify({ at: Date.now(), sources } satisfies CachedSources))
  } catch {
    /* quota — non-fatal */
  }
}
