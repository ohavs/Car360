...
import { AppBar, Button, Card, EmptyState, PhotoViewer, Screen, Skeleton, StatusChip, Text, Touchable } from '../../../ui'
...
export default function InsuranceScreen() {
  ...
  return (
    <Screen header={<AppBar title="ביטוחים" subtitle={car ? carDisplayName(car) : undefined} back />}>
      {loading ? (
        [0, 1].map((i) => <Skeleton key={i} height={110} radius={radius.card} />)
      ) : sorted.length === 0 ? (
        <EmptyState icon={Shield} title="אין פוליסות ביטוח" subtitle="פוליסות שתוסיפו יופיעו כאן, עם תזכורת לפני שהן נגמרות" />
      ) : (
        sorted.map((p) => {
