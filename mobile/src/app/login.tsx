import { useState } from 'react'
import { StyleSheet, View } from 'react-native'
import Svg, { Path } from 'react-native-svg'
import { useSafeAreaInsets } from 'react-native-safe-area-context'
import { SignInError, useAuth } from '../features/auth/AuthProvider'
import { useTheme } from '../theme/ThemeProvider'
import { radius, space } from '../theme/tokens'
import { Text, Touchable } from '../ui'
import { CarSilhouette } from '../ui/CarSilhouette'

const FEATURES = ['טיפולים', 'ביטוחים', 'טסט', 'מסמכים', 'תזכורות', 'שיתוף']

export default function LoginScreen() {
  const { signIn } = useAuth()
  const { colors } = useTheme()
  const insets = useSafeAreaInsets()
  const [busy, setBusy] = useState(false)
  const [error, setError] = useState<string | null>(null)

  const onSignIn = async () => {
    setBusy(true)
    setError(null)
    try {
      await signIn()
    } catch (e) {
      setError(e instanceof SignInError ? e.message : 'הכניסה נכשלה. נסו שוב.')
    } finally {
      setBusy(false)
    }
  }

  return (
    <View
      style={[
        styles.root,
        { backgroundColor: colors.background, paddingTop: insets.top + space.xxxl, paddingBottom: insets.bottom + space.xxl },
      ]}
    >
      <View style={styles.hero}>
        <View style={[styles.brandPill, { backgroundColor: colors.surface, borderColor: colors.outline }]}>
          <View style={[styles.dot, { backgroundColor: colors.brand }]} />
          <Text variant="label">Car360</Text>
        </View>
        <Text variant="display" align="center" style={styles.headline}>
          כל הרכבים שלך.
        </Text>
        <Text variant="display" tone="brand" align="center">
          מקום אחד.
        </Text>
        <View style={styles.features}>
          {FEATURES.map((f) => (
            <View key={f} style={[styles.chip, { backgroundColor: colors.surface, borderColor: colors.outline }]}>
              <Text variant="label" tone="onSurfaceVariant">
                {f}
              </Text>
            </View>
          ))}
        </View>
      </View>

      <View style={styles.car}>
        <CarSilhouette width={300} />
      </View>

      <View style={styles.bottom}>
        {error && (
          <View style={[styles.error, { backgroundColor: colors.dangerContainer }]}>
            <Text variant="label" tone="danger" align="center">
              {error}
            </Text>
          </View>
        )}
        <Touchable
          onPress={() => void onSignIn()}
          disabled={busy}
          accessibilityRole="button"
          accessibilityLabel="כניסה עם Google"
          accessibilityState={{ busy }}
          rippleColor="rgba(255,255,255,0.2)"
          style={[styles.google, { backgroundColor: colors.primary }, busy && styles.busy]}
        >
          <View style={styles.googleLogo}>
            <GoogleLogo />
          </View>
          <Text variant="bodyStrong" style={{ color: colors.onPrimary }}>
            {busy ? 'מתחבר…' : 'כניסה עם Google'}
          </Text>
        </Touchable>
        <Text variant="caption" tone="muted" align="center">
          הנתונים שלך מסונכרנים ומאובטחים בחשבון Google שלך
        </Text>
      </View>
    </View>
  )
}

function GoogleLogo() {
  return (
    <Svg width={16} height={16} viewBox="0 0 48 48">
      <Path fill="#EA4335" d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38 30.47 0 24 0 14.62 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.72 17.74 9.5 24 9.5z" />
      <Path fill="#4285F4" d="M46.98 24.55c0-1.57-.15-3.09-.38-4.55H24v9.02h12.94c-.58 2.96-2.26 5.48-4.78 7.18l7.73 6c4.51-4.18 7.09-10.36 7.09-17.65z" />
      <Path fill="#FBBC05" d="M10.53 28.59c-.48-1.45-.76-2.99-.76-4.59s.27-3.14.76-4.59l-7.98-6.19C.92 16.46 0 20.12 0 24c0 3.88.92 7.54 2.56 10.78l7.97-6.19z" />
      <Path fill="#34A853" d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.15 1.45-4.92 2.3-8.16 2.3-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.62 48 24 48z" />
    </Svg>
  )
}

const styles = StyleSheet.create({
  root: {
    flex: 1,
    paddingHorizontal: space.xxl,
  },
  hero: {
    alignItems: 'center',
  },
  brandPill: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: space.sm,
    borderRadius: radius.full,
    borderWidth: StyleSheet.hairlineWidth,
    paddingHorizontal: space.lg,
    paddingVertical: space.sm,
  },
  dot: {
    width: 8,
    height: 8,
    borderRadius: 4,
  },
  headline: {
    marginTop: space.xxl,
  },
  features: {
    flexDirection: 'row',
    flexWrap: 'wrap',
    justifyContent: 'center',
    gap: space.sm,
    marginTop: space.xxl,
  },
  chip: {
    borderRadius: radius.full,
    borderWidth: StyleSheet.hairlineWidth,
    paddingHorizontal: space.md,
    paddingVertical: 6,
  },
  car: {
    flex: 1,
    alignItems: 'center',
    justifyContent: 'center',
  },
  bottom: {
    gap: space.md,
  },
  error: {
    borderRadius: radius.field,
    padding: space.md,
  },
  google: {
    minHeight: 56,
    borderRadius: radius.full,
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
    gap: space.md,
  },
  busy: {
    opacity: 0.7,
  },
  googleLogo: {
    width: 28,
    height: 28,
    borderRadius: 14,
    backgroundColor: '#ffffff',
    alignItems: 'center',
    justifyContent: 'center',
  },
})
